...
/* maximum runtime for each benchmark */
    ...
    #define BENCH_MIN_RUNTIME_SEC   3.0F
#endif
...
void bench_hpke_exchange(void) {
	...
	switch(kem){
		...
		case(DHKEM_X25519_HKDF_SHA256):
			...
			break;
		case(KEM_KYBER512_HKDF_SHA256):
			printf("%s\n", "KEM_KYBER512_HKDF_SHA256");
			break;
		case(KEM_KYBER768_HKDF_SHA384):
			printf("%s\n", "KEM_KYBER768_HKDF_SHA384");
			break;
		case(KEM_KYBER1024_HKDF_SHA512):
			printf("%s\n", "KEM_KYBER1024_HKDF_SHA512");
			break;
	}
	...
	ret = wc_HpkeInit(hpke, kem, hkdf, aead, NULL);
	...
	/* generate the keys */
    if (hpke->kem != KEM_KYBER512_HKDF_SHA256
    		&& hpke->kem != KEM_KYBER768_HKDF_SHA384
			&& hpke->kem != KEM_KYBER1024_HKDF_SHA512) {

    	/* Allocate the pubKey */
    	pubKey = XMALLOC(HPKE_Npk_MAX, hpke->heap, DYNAMIC_TYPE_KEY);
    	pubKeySz = HPKE_Npk_MAX;
		...
		/* Allocate the pubKey for PQ */
		switch(hpke->kem) {
			case(KEM_KYBER512_HKDF_SHA256):
				pubKey = XMALLOC(sizeof(byte)*KYBER512_CIPHER_TEXT_SIZE,
					hpke->heap, DYNAMIC_TYPE_KEY);
				pubKeySz = KYBER512_CIPHER_TEXT_SIZE;

				ephemeralKey = XMALLOC(sizeof(byte)*KYBER512_CIPHER_TEXT_SIZE,
									hpke->heap, DYNAMIC_TYPE_KEY);
				break;
			case(KEM_KYBER768_HKDF_SHA384):
				pubKey = XMALLOC(sizeof(byte)*KYBER768_CIPHER_TEXT_SIZE,
					hpke->heap, DYNAMIC_TYPE_KEY);
				pubKeySz = KYBER768_CIPHER_TEXT_SIZE;

				ephemeralKey = XMALLOC(sizeof(byte)*KYBER768_CIPHER_TEXT_SIZE	,
									hpke->heap, DYNAMIC_TYPE_KEY);
				break;
			case (KEM_KYBER1024_HKDF_SHA512):
				pubKey = XMALLOC(sizeof(byte)*KYBER1024_CIPHER_TEXT_SIZE,
					hpke->heap, DYNAMIC_TYPE_KEY);
				pubKeySz = KYBER1024_CIPHER_TEXT_SIZE;

				ephemeralKey = XMALLOC(sizeof(byte)*KYBER1024_CIPHER_TEXT_SIZE,
									hpke->heap, DYNAMIC_TYPE_KEY);
				break;
		}
	}
    ...
    do {
    	 ...
    	 /* -------------------------------------------------------------------------------------- */
		/* This part is the benchmarkable one */
	    ...
	    if (hpke->kem != KEM_KYBER512_HKDF_SHA256
			...
			/* copy ciphertext to pubKey */
			XMEMCPY(pubKey, ephemeralKey, pubKeySz);
		}
		...
		if (ret != 0) {
			printf("Error occured during hpke exchange...\n");
			printf("ret = %d\n", ret);
			goto exit;
		}
		 /* -------------------------------------------------------------------------------------- */
		...
		// Comment out this one for proper benchmark
		break;

        count += 1;
    } while (bench_stats_check(start));
    bench_stats_asym_finish("HPKE", 1, desc[10], 0, count, start, ret);
...
if (hpke->kem != KEM_KYBER512_HKDF_SHA256
		...
		XFREE(ephemeralKey, hpke->heap, NULL);
	}

	if (pubKey != NULL)
		XFREE(pubKey, hpke->heap, NULL);
...
int wolfcrypt_benchmark_main(int argc, char** argv)
{
    ...
    while (argc > 1) {
        ...
        else if (string_matches(argv[1], "-hpke")) {
        	bench_hpke = 1;
        	bench_all = 0;
        	optMatched = 1;
        }
